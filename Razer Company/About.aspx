<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="About.aspx.cs" Inherits="Razer_Company.About" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server"> 
    <div class="container">
    <h1>About Us</h1>
        <img src="Image/CEO%20Min%20Liang%20Tan.jpg" />
        <p>Razer was found in 2005 in San Diego,California by CEO Min-Liang Tan , what began as a tiny shared office of gamers became a mission to create high quality products designed solely for the need of gamers</p>
        <p>Razer is known for its pursuit of perfection in introducting innovative connected devices and software. With the use of cutting edge technology and usage by top Esports athletes with constant testing, quickly resulted in competitive and abundant of experience that is designed
        for gamers,by gamers.</p>
        <p>For years,Razer has turned into a gaming lifestyle that connects gamers all over the years, without the regards about age, gaming genre and console. Razer has commitment to the gaming community which can be seen in its sponsorship towards gaming event, host Esport Event and group of 
            professional Esports athletes.As the industry grows,razer is position firmly to develop more innovative technology for gamers, by gamers.</p>
    </div>
</asp:Content>
